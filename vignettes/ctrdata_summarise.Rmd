...
# $ _id                                   : chr  "2004-004386-15-DE" "2005-000915-80-IT" "2005-001267-63-IT" ...
...
# $ n_date_of_competent_authority_decision: Date, format: "2005-07-08" "2005-04-21" "2005-07-08" "2006-05-26" ...
...
if (FALSE) {
  db <- nodbi::src_sqlite(collection = "x")
  ctrLoadQueryIntoDb(
...
# number of paticipants (last number is typicall all groups summed up)
result$totalparticipants <- vapply(
  result[["clinical_results.baseline.analyzed_list.analyzed.count_list.count.value"]],
  FUN = function(x) as.numeric(rev(strsplit(x, " / ")[[1]])[1]), numeric(1L))

result$clinical_results.baseline.analyzed_list.analyzed.count_list.count.value[1:3]
...
result$pvalueprimaryanalysis <- vapply(
  ...
  FUN = function(x) as.numeric(strsplit(x, " / ")[[1]][1]), numeric(1))
...
result$controlled <- grepl("(Placebo|No Intervention)", 
...
result[["startdatefirst"]] <- dfMergeVariablesRelevel(
  df = result,
  colnames = c(
    "x",
    "x6_date_on_which_this_record_was_first_entered_in_the_eudract_database"
