...
The command line tools `perl`, `sed` and `cat` are required for `ctrLoadQueryIntoDb()`, the main function of package `ctrdata`, but *only* for register "EUCTR". No other function in this package and no other register has this requirement.
...
- For convenience, install a script to automatically copy user's query from web browser, see [here](https://github.com/rfhb/ctrdata#id_3-script-to-automatically-copy-users-query-from-web-browser)

Command line tools `perl`, `cat` and `sed` are required for `ctrLoadQueryIntoDb()`, the main function of package `ctrdata`, to work with the register "EUCTR" (but are *not* required for any other function or for `ctrdata` to work with the registers CTIS, CTGOV, CTGOV2 or ISRCTN, since 2023-11-21).
