...
For this requirement on MS Windows, the [`Cygwin`](https://cygwin.org/install.html) environment be installed, into the local directory `c:\cygwin` (or any folder corresponding to `c:\cygw*`). The installation of a minimal Cygwin environment on MS Windows can be done with the help of package `ctrdata` in R as follows: 
...
The command line tools `perl`, `sed` and `php` (5.2 or higher) are typically installed and available by default in Linux and macOS systems. 

- In macOS, [`homebrew`](https://brew.sh/) can be used to install `php` which may be needed starting from macOS 12 Monterey: first install [`homebrew`](https://brew.sh/) and then run on the command line: `brew install php`. 

- In Linux, tools for installation vary by distribution (e.g., `sudo apt install php php-xml php-json`)

## Databases to use

At this time, a PostgreSQL, an SQLite or a remote or local MongoDB database can be used with the package `ctrdata`. SQLite is provided in the R package [`RSQLite`](https://rsqlite.r-dbi.org/). Suggested installation instructions for a local MongoDB server are [here](https://docs.mongodb.com/manual/administration/install-community/); a remote MongoDB database server is accessible [here](https://www.mongodb.com/cloud/atlas). 
