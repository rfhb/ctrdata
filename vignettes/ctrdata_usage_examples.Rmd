...
getKeyNestedList <- function(thelist, thekey) {
  ...
  # perhaps the list is still a character string
  if (inherits(x = thelist, what = "character") && 
      all(sapply(thelist, function(x) substr(x, 1, 4) == "list"))) {
    thelist <- lapply(thelist, function(x) eval(parse(text = x)))
  }
...
# Nested element is a list as a character string,
# which requires eval(parse()) to be accessed 
# and this is exemplified in the helper function. 
result[1, "b1_sponsor"]
...
tmp <- lapply(tmp, function(x) eval(parse(text = x)))

# change into number
tmp <- lapply(tmp, function(x) sum(as.numeric(unlist(x)), na.rm = TRUE))

# remove empty list elements
tmp <- sapply(tmp, function(x) ifelse(length(x), x, NA))

# check that unit is participants, not e.g. wounds or eyes
tmpparticipants <- sapply(result$clinical_results.baseline.analyzed_list.analyzed.units, 
              function(x) all(eval(parse(text = x))$units == "Participants"))
...
# two or more arms?
tmp <- lapply(result$arm_group.arm_group_type, 
              FUN = function(x) eval(parse(text = x)))
