...
tf <- function() {
  ...
  expect_equivalent(c(
    ...
    "https://clinicaltrials.gov/search?cond=cancer&intr=(antibody&start=2000-01-01_2030-01-01&primComp=2000-01-01_2030-01-01&term=AREA[DesignPrimaryPurpose](DIAGNOSTIC OR PREVENTION OR TREATMENT)&aggFilters=studyType:int) AND (Drug OR Biological),results:with",
  ...
  expect_equivalent(c(
    ...
    "https://clinicaltrials.gov/search?&intr=(bispecific&start=_2030-01-01&term=AREA[DesignPrimaryPurpose](DIAGNOSTIC OR PREVENTION OR TREATMENT)&aggFilters=studyType:int) AND (Drug OR Biological)",
    "https://clinicaltrials.gov/expert-search?term=AREA[InterventionSearch]\"bispecific\" AREA[StartDate]RANGE[MAX,2030-01-01] AND (AREA[StudyType]INTERVENTIONAL) AND (AREA[DesignPrimaryPurpose](DIAGNOSTIC OR PREVENTION OR TREATMENT)) AND (AREA[InterventionSearch](DRUG OR BIOLOGICAL))",
  ...
  expect_equivalent(c(
    ...
    "https://clinicaltrials.gov/search?term=\"antibody\" AND \"covid\"&intr=Drug OR Biological&term=AREA[DesignPrimaryPurpose](DIAGNOSTIC OR PREVENTION OR TREATMENT)&aggFilters=status:com,studyType:int",
  ...
  expect_equivalent(c(
    ...
    "https://clinicaltrials.gov/search?term=\"cancer institute\"&intr=Drug OR Biological&term=AREA[DesignPrimaryPurpose](DIAGNOSTIC OR PREVENTION OR TREATMENT)&aggFilters=studyType:int",
