...
The motivation is to understand trends in design and conduct of trials, their availability for patients and their detailled results. The package is to be used within the [R](https://www.r-project.org/) system; this README was reviewed on 2021-12-01 for v1.8.0.9000.
...
* Retrieved (downloaded) trial information is transformed and stored in a document-centric database, for fast and offline access. Uses `PostgreSQL` (`r emo::ji("bell")`new in v1.8.0.9000), `RSQLite` or `MongoDB` as databases, via R package `nodbi`. Easily re-run a previous query to update a database.  
