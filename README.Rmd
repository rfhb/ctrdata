...
Remember to respect the registers' terms and conditions (see `ctrOpenSearchPagesInBrowser(copyright = TRUE)`). Please cite this package in any publication as follows: "Ralf Herold (2025). _ctrdata: Retrieve and Analyze Clinical Trials in Public Registers._ R package version 1.22.0, https://cran.r-project.org/package=ctrdata". 
...
Additionally, this script retrieves results for `CTIS` when opening search URLs such as [https://euclinicaltrials.eu/ctis-public/search#searchCriteria={"status":[3,4]}](https://euclinicaltrials.eu/ctis-public/search#searchCriteria={"status":[3,4]}). 
...
* Information on trial registers and how they can be used with `ctrdata` (last updated 2025-03-09):
```{r help_registers}
