...
Last checked and updated on 2019-10-13 for version 0.20, with *breaking changes*, bug fixes and new features: 

* minimised dependencies: works now with `RSQLite` (>= 2.1.2), local and remote MongoDB servers, via R package `nodbi` (>= 0.3). This is a breaking change that could not be avoided in order to generalise the database access, and it was made possible by introducing a REGEXP operator into [RSQLite](https://github.com/r-dbi/RSQLite/pull/296) and adding a set of methods to `nodbi` based on the Json1 extension of [SQLite](https://github.com/ropensci/nodbi/pull/25). 
...
Package `ctrdata` can be found [here on CRAN](https://cran.r-project.org/package=ctrdata) and  [here on github](https://github.com/rfhb/ctrdata). Within [R](https://www.r-project.org/), use the following commands to get and install package `ctrdata`:
