...
In any backend `NoSQL` [database](https://rfhb.github.io/ctrdata/index.html#databases-for-use-with-ctrdata), one clinical trial is one document (and one row in `SQLite`, `PostgreSQL`, `DuckDB`). The `NoSQL` backends allow different documents to have different structures. Package `ctrdata` stores in every such document: 
...
- all original fields exactly as provided by the register for that trial (see examples [below](https://rfhb.github.io/ctrdata/index.html#trial-records-json-in-databases))
