...
The motivation is to understand trends in design and conduct of trials, their availability for patients and their detailled results. The package is to be used within the [R](https://www.r-project.org/) system; this README was reviewed on 2021-12-16 for v1.8.0.9002.
...
- In macOS including up to 11 Big Sur, these are already installed; `r emo::ji("bell")`for macOS 12 Monterey and in case of errors, [`homebrew`](https://brew.sh/) can be used to install `php`. 
...
Under the hood, scripts `euctr2json.sh` and `xml2json.php` (in `ctrdata/exec`) transform EUCTR plain text files and CTGOV `XML` files to `ndjson` format, which is imported into the database. The database is specified first, using `nodbi` (using `PostgreSQL`, `RSQlite` or `MongoDB` as backend); then, trial information is retrieved and loaded into the database: 
