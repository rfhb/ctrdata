...
The motivation is to understand trends in design and conduct of trials, their availability for patients and their detailled results. The package is to be used within the [R](https://www.r-project.org/) system; this README was reviewed on 2021-12-23 for v1.8.0.9002.
...
- In macOS including 11 Big Sur, these are already installed; `r emo::ji("bell")`for macOS 12 Monterey and in case of errors, [`homebrew`](https://brew.sh/) can be used to install `php`. 
...
`dfTrials2Long()` `r emo::ji("bell")` | Transform the data.frame from `dbGetFieldsIntoDf()` into a long name-value data.frame, including deeply nested fields
`dfName2Value()` `r emo::ji("bell")` | From a long name-value data.frame, extract values for variables (fields) of interest (e.g., endpoints)
