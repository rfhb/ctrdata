...
* Analysis can be done with `R` (using `ctrdata` convenience functions) or others systems. Unique (de-duplicated) trial records are identified across registers. `ctrdata` can merge and recode information (fields) and also provides easy access even to deeply-nested fields (new in version 1.4).
...
`dfMergeTwoVariablesRelevel()` `r emo::ji("timer")`  | Deprecated - Merge two simple variables into a new variable, optionally map values to a new set of values
