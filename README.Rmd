...
### 2. External tools (`r emoji::emoji("bell")`only needed for EUCTR)

These are required for `ctrLoadQueryIntoDb()`, the main function of package `ctrdata` (see [Example workflow](#example-workflow)), to work with the register EUCTR (but are *not* required for any other function or for `ctrdata` to work with the registers CTIS, CTGOV, CTGOV2 or ISRCTN, since 2023-11-21).
...
- In macOS including 11 Big Sur, these are already installed; as alternative and `r emoji::emoji("bell")`for macOS 12 Monterey and later, [`homebrew`](https://brew.sh/) can be used: `brew install php`, which seems to include all `php` libraries required for `ctrdata`.
...
* Replace external command line tools still needed for EUCTR, CTGOV, ISRCTN, likely by V8 and custom Javascript
