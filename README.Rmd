...
* Downloaded trial information is stored in a document-centric database, for fast and offline access. Information from different registers can be accumulated in a single collection. Uses `RSQLite`, `DuckDB`, `PostgreSQL`, `MariaDB` or `MongoDB`, see [Databases](#databases-that-can-be-used-with-ctrdata). Interactively browse through trial structure and data. Easily re-run a previous query to update trial records. 
...
<blockquote>
```{r, eval=TRUE, results='asis', echo=FALSE, warning=FALSE}
citation("ctrdata") |> 
  format(style = "text") |> 
  sub("????", packageDate("ctrdata"), x = _, fixed = TRUE) |> 
  rev() |> 
  cat(sep = " or <br/>")
```
</blockquote>
...
-->
- Machado et al. (2026) The road for developing new pharmacological therapies for Parkinson's disease: Current trends and targets in clinical trials. Journal of Parkinson's disease
...
- Cancer Research UK (2017) The impact of collaboration: The value of UK medical research to EU science and health. [link](https://acmedsci.ac.uk/file-download/72060732)
