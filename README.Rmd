...
* Retrieved (downloaded) trial information is transformed and stored in a document-centric database, for fast and offline access. Uses `PostgreSQL` (`r emo::ji("bell")`new in v1.8.0.9000), `RSQLite` or `MongoDB` as databases, via R package `nodbi`: see section [Databases](#databases-that-can-be-used-with-ctrdata) below. Easily re-run a previous query to update a database.
